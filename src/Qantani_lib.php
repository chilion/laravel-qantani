...
class Qantani_lib
{
    public static $endpoint = 'https://www.qantanipayments.com/api/'; /** location of the api */
    public static $version = '1.2.4'; /** PHP class version */
    ...
    public static function CreateInstance($merchant_id, $merchant_key, $merchant_secret)
    {
        $instance = new self();
    ...
    {
        $this->_mandatory($parameters, []);
        $data = $this->_call('GETPAYMENTMETHODS', $parameters, ['Response', 'Response', 'PaymentMethods', 'PaymentMethod']);
        $res = [];
        foreach ($data as $item) {
            if ($item['Active']['value'] == 'Y') {
                $res[] = ['name' => $item['Name']['value']];
            }
        }
    ...
    public function Ideal_getBanks($parameters = [])
    {
        $this->_mandatory($parameters, []);
        $data = $this->_call('IDEAL.GETBANKS', $parameters, ['Response', 'Banks', 'Bank']);
        $res = [];
        foreach ($data as $item) {
            $res[] = [
    ...
    {
        $this->_mandatory($parameters, ['Amount', 'Currency', 'Description', 'Bank', 'Return']);

        return $this->_call('IDEAL.EXECUTE', $parameters, ['Response', 'Response', 'BankURL', 'value']);
    }
    ...
    {
        $this->_mandatory($parameters, ['Amount', 'Currency', 'Description', 'Return']);

        return $this->_call('CC.EXECUTE', $parameters, ['Response', 'Response', 'BankURL', 'value']);
    }
    ...
    public function Sofort_execute($parameters = [])
    {
        $this->_mandatory($parameters, ['Amount', 'Currency', 'Description', 'Return']);

        return $this->_call('SOFORT.EXECUTE', $parameters, ['Response', 'Response', 'BankURL', 'value']);
    }
    ...
    {
        $this->_mandatory($parameters, ['Amount', 'Currency', 'Description']);

        return $this->_call('BITPAY.EXECUTE', $parameters, ['Response', 'Response', 'BankURL', 'value']);
    }
    ...
    public function Paypal_execute($parameters = [])
    {
        $this->_mandatory($parameters, ['Amount', 'Currency', 'Description', 'Return']);

        return $this->_call('PAYPAL.EXECUTE', $parameters, ['Response', 'Response', 'BankURL', 'value']);
    }
    ...
    public function paylater_execute($parameters)
    {
        ...
        return $this->_call('PAYLATER.EXECUTE', $parameters, ['Response', 'Status', 'value']);
    }
    ...
    {
        $this->_mandatory($parameters, ['TransactionID', 'Amount', 'TransactionCode']);

        return $this->_call('REFUND', $parameters, ['Response', 'Response', 'TransactionID', 'value']);
    }
    ...
    {
        $this->_mandatory($parameters, ['TransactionID', 'TransactionCode']);
        $result = $this->_call('TRANSACTIONSTATUS', $parameters, ['Response', 'Transaction']);
    ...
    private function _call($function, $parameters, $look_for = [])
    {
        $this->_last_error = '';
